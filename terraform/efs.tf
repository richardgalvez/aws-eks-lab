resource "aws_efs_file_system" "eks" {
  ...
  # lifecycle_policy {
  #  transition_to_ia = "AFTER_30_DAYS"     # S3 Lifecycle transition policy to Infrequent Access
  # }
}
...
# For reference/optional, since we already have the init helm provider (making it self-contained)
data "aws_eks_cluster" "eks_v2" {
  name = aws_eks_cluster.eks.name
}

# Also optional, same as above
