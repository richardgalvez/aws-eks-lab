resource "helm_release" "secrets_csi_driver" {
  ...
  # Must be set if you use env variables, it will create additional RBAC policies needed to use them
  set {
...
data "aws_iam_policy_document" "myapp_secrets" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:secret-ns:myapp"]
    }
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        # Don't "*" use in production. Use the AWS Secret's specific ARN, something like: "arn:*:secretsmanager:*:*:secret:my-secret-kkargS"
        Resource = "*"
      }
    ]
  })
